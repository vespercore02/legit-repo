...
sub pushover {
	...
	$server =~ s/International - iRO: //;
	$final_message .= $server." - ".$config{username};
	require LWP::UserAgent;
	LWP::UserAgent->new()->post(
	  'https://api.pushover.net/1/messages.json' , [
	  "token" => 'a3x4gyvcd6is6xw5eafiaggaepqixs',
	  "user" => 'uwxat1b3fad5c4ps6aga3remcqkum1',
...
my %server_name_lut = ('128.241.92.133:6800' => 'Odin');
...
sub core_start3 {
	...
	message sprintf("Loading %s... \n", 'control\_koreShield\koreShield_'.$server_name_lut{$master->{ip}.':'.$master->{port}}.'.txt');
	&RevokUtils::Parsers::parseSectionedFile('control\_koreShield\koreShield_'.$server_name_lut{$master->{ip}.':'.$master->{port}}.'.txt', \%core_databases);
...
sub detectGM_someonesMuted {
	if (defined $field && $field && !$field->isCity()) {
		error sprintf("Someone was muted out of town! \n"), "koreShield_detect";
...
sub detectGM_forced_status {
	...
	if (($args->{opt1} == 4) && ($args->{ID} eq $accountID)) {
		error sprintf("Status Forced Sleep! \n"), "koreShield_detect";
...
sub bus_isStarted {
	...
	if (!$bus) {
		die("You MUST start a BUS server and configure each OpenKore bot to use it. Open the control/sys.txt file and set bus 0 to bus 1. \n\n");
	} elsif (!$bus_server) {
		$bus_server = $bus->onMessageReceived->add(undef, \&bus_parseMsg);
		Plugins::delHook($networkHook);
		undef $networkHook;
...
sub detectGM_analyseSkillCaster {
	...
	if ($caller eq 'is_casting') {
		if (isIn_Array(unpack("V1", $args->{sourceID}), \@{$core_databases{GMIDS}})) {
			error sprintf("Player ID %s blacklist is casting %s! Disconnecting... \n", unpack("V1", $args->{sourceID}), $skillname), "koreShield_detect";
			...
			if ($castername->{name} =~ /^Unknown \#\d+/ || !$castername->{name}) {
				error sprintf("Unknown Player %s (%s) It's buffing you with %s! Disconnecting...\n", $castername->{name}, unpack("V", $args->{sourceID}), $skillname), "koreShield_detect";
			...
			error sprintf("Unknown Player %s (%s) is casting %s on you ! Disconnecting...\n", $castername->{name}, unpack("V", $args->{sourceID}), $skillname), "koreShield_detect";
		...
		if (AI::action eq "attack") {
			...
			#warning "lol ".$monstersList->getByID($args->{sourceID})."\n" if $monstersList->getByID($args->{sourceID});
				if ((grep {$_ eq $args->{skillID}} (73, 2051)) && !($monstersList->getByID($args->{sourceID}))) { # do not detect 28 (Heal) here
					error sprintf("Player %s (%s) is casting %s on your monster!  Disconnecting...\n", $castername->{name}, unpack("V", $args->{sourceID}), $skillname), "koreShield_detect";
		...
		if ($args->{sourceID} && (isIn_Array(unpack("V1", $args->{sourceID}), \@{$core_databases{GMIDS}}))) {
			error sprintf("Player ID %s blacklist used %s! Disconnecting... \n", unpack("V1", $args->{sourceID}), $skillname), "koreShield_detect";
			&core_eventsReaction('blacklisted_used_skill');
			return;
		} elsif ($castername =~ /^(NPC|Player)? ?\[?GM\]?.*/) {
			error sprintf("%s with [GM] on nick used %s! Disconnecting... \n", $castername->{name}, $skillname), "koreShield_detect";
		...
		if (unpack("V1", $args->{targetID}) eq unpack("L1", $accountID)) { # target skills
			if ($args->{skillID} == 29 || $args->{skillID} == 34 || $args->{skillID} == 361) { # alguns buffs menos kyrie
				if ($castername =~ /^Unknown \#\d+/ || !$castername->{name}) {
					error sprintf("Unknown Player %s (%s) buffed you with %s! Disconnecting...\n", $castername->{name}, unpack("V", $args->{sourceID}), $skillname), "koreShield_detect";
					&core_eventsReaction('unknown_buffed_me');
					return;
				}
				...
				error sprintf("Unknown Player %s (%s) used %s in you ! Disconnecting...\n", $castername->{name}, unpack("V", $args->{sourceID}), $skillname), "koreShield_detect";
				&core_eventsReaction('unknown_used_skill_me');
				return;
			}
		} else { # ground skills
			if ($args->{skillID} == 70) { # santu�rio
				...
				if ($config{master} =~ /(Thor|Revok)/) {
					error sprintf("Player %s (%s) used a portal on a server not allowed! Disconnecting...\n", $castername->{name}, unpack("V", $args->{sourceID})), "koreShield_detect";
		...
		if (AI::action eq "attack") {
			...
			if (($monsterID eq $args->{targetID}) && AI::args->{dmgFromYou_last} && $args->{sourceID}) {
				...
				if (
					...
					error sprintf("Player %s (%s) has used %s on your monster! Disconnecting...\n", $castername->{name}, unpack("V", $args->{sourceID}), $skillname), "koreShield_detect";
					&core_eventsReaction('slaving_monster');
				}
			}
		}
...
sub detectGM_perfectHide {
	...
	my $msg;
	$msg .= sprintf("A GM in perfect hide (%s) has been detected! Disconnecting...\n", $args->{actor}->{name});
	$msg .= "=================== A GM HAS BEEN DETECTED ==================\n";
...
sub detectGM_handleLogin {
	 # 4	account blocked - most common in privates
	 # 6	banned by time - most common in officers, iRO
	 # 15	GM gave you a kick
	 # 101	usually banned by many connections
	 # 102	the same as 101, but unusual in official
	my (undef, $args) = @_;
...
sub core_actorInfo {
	...
	if (isIn_Array($ID, \@{$core_databases{GMIDS}}) eq 1) {
		...
		$detect_reason = sprintf("Headgear top (%s) on blacklistt", $player->{headgear}{top});
		...
		$detect_reason = 'Headgear mid on blacklist';
...
*Misc::updatePlayerNameCache = sub {
	...
	# First, cleanup the cache. Remove entries tHeadgear are too old.
...
sub core_eventsReaction {
	...
	if ($bus_args) {
		...
		if ($bus_args->{mapserver}) {
			...
			warning sprintf("%s detected an GM in this map server !\n", $bus_args->{player}), "koreShield";
		} elsif ($bus_args->{map}) {
